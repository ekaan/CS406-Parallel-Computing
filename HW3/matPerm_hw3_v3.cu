...
__global__
void perMat (float* x, float* x_pre_d, int rowSize, long long* myMat, float* p, long long tNum) {
  ...
  float* s_x = arr + threadIdx.x * rowSize;
  ...
  */
  ...
  for (long long i = start_loc; i < end_loc; i++) {
    ...
    for(int j = 0 ; j < rowSize ; j++) {
      s_x[j] += myMat[bit_id * rowSize + j] * s;
      prod_x *= s_x[j];
    }
...
int main(int argc, const char** argv)
{
  ...
  float p = 1;
  float lastColumn, t_sum;

  float* x_pre_h = new float [N];
  for(int i = 0 ; i < N ; i++)
  {
...
*/
  float* x_pre_d;
  cudaMalloc((void**)&x_pre_d, N*sizeof(float));
  cudaMemcpy(x_pre_d, x_pre_h, sizeof(float)*N, cudaMemcpyHostToDevice);

  float* x = new float [N*th_num];
  float* x_k;
  cudaMalloc((void**)&x_k, th_num*N*sizeof(float));
  //  cudaMemcpy(x_k, x, sizeof(float)*N*th_num, cudaMemcpyHostToDevice);

  float* p_ptr;
  cudaMalloc((void**)&p_ptr, sizeof(float));
  cudaMemcpy(p_ptr, &p, sizeof(float), cudaMemcpyHostToDevice);
  ...
  */
  ...
  int totalSize = 256*sizeof(float)*N;

  perMat<<<my_grid,my_block,totalSize>>>(x_k, x_pre_d, N, myMat, p_ptr, th_num);
  cudaDeviceSynchronize();
  cudaCheckError();
  cout << "1 | " << p << endl;

  cudaMemcpy(&p, p_ptr, sizeof(float), cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
  cout << "2 | " << p << endl;

  float result = (4 * (N & 1) - 2) * p;
  //// ITERATION ENDS HERE/////---------------------------------------------
 end = omp_get_wtime();
